#!/usr/bin/env bash
# MVS3-C 验收一键跑：前置检查 → 起 pgvector 测试容器（若无）→ 构建瘦身镜像
# oneai-engine:mvs3c（--no-default-features，无 ONNX 链）→ 构建宿主机侧二进制
# （带 postgres feature，编排器进程用）→ 跑 mvs3c_verify.mjs → 兜底清理。
#
# 用法（仓库根目录）：
#   ./deploy/docker/mvs3c_run.sh [--bin target/debug/oneai] [--release] [--keep]
#
# 前置：docker daemon（colima 即可）、~/.oneai/config.toml、node ≥18、
#      platforms/web/node_modules/ws（npm i 于 platforms/web）、
#      platforms/web/dist 为**当前源码构建**（npm run build——镜像 COPY 它）。
...
[[ -f platforms/web/dist/index.html ]] || { echo "缺 platforms/web/dist — cd platforms/web && npm run build"; exit 2; }
...
echo "── 构建瘦身镜像 oneai-engine:mvs3c（--no-default-features，首次全量较久）──"
docker build -f deploy/docker/Dockerfile -t oneai-engine:mvs3c .
docker image inspect oneai-engine:mvs3c --format '   镜像尺寸: {{.Size}} bytes'
...
echo "── 跑 MVS3-C 验收 ──"
set +e
node deploy/docker/mvs3c_verify.mjs --bin "$BIN" "${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}"
RC=$?
set -e
