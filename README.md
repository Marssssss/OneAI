...
- **多 Agent 原生** — 模型驱动的 SubAgent 分层委托（`delegate` 元工具，一轮多委托 + 依赖感知并行波次调度）+ 范式切换（`switch_paradigm` 进入 Plan/Reflect/Explore 图流）+ 引擎级 **GroupChat 原语**驱动场景化多角色对话；聚合/路由/辩论等编排模式由确定性 StateGraph 表达。
...
> 历史上的 Team/Swarm/Handoff 编排层已移除——其聚合/路由/辩论等模式可由 `delegate` + 确定性 StateGraph 直接表达，避免与主 Loop 的委托/范式机制能力重复。
