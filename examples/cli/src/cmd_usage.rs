...
pub fn cmd_usage_report() {
    ...
    let summary = rt
        .block_on(tracker.global_usage())
        .expect("Failed to get global usage");
...
pub fn cmd_usage_session(id: &str) {
    let tracker = InMemoryUsageTracker::new();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let session_usage = rt
        .block_on(tracker.session_usage(id))
        .expect("Failed to get session usage");
    let by_model = rt
        .block_on(tracker.usage_by_model(id))
        .expect("Failed to get per-model usage");
...
pub fn cmd_usage_export(format: &str) {
    ...
    let rt = tokio::runtime::Runtime::new().unwrap();
    let records = rt
        .block_on(tracker.global_records())
        .expect("Failed to get usage records");
