...
pub fn draw_approval_card(f: &mut Frame, rect: Rect, pending: &ApprovalPendingState) {
    ...
    let content = format!(
        ...
        request.tool_name, perm_label, request.justification,
    );
...
pub fn render_approval_card_inline(
    content: &str,
    max_width: usize,
    ...
    let mut action_spans = vec![Span::styled("│ ", Style::default().fg(APPROVAL_BORDER))];
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            ...
            Style::default().fg(APPROVAL_BORDER),
        ),
        Span::styled("┘", Style::default().fg(APPROVAL_BORDER)),
    ]));
