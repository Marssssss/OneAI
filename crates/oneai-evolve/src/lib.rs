...
//! - **E1** (this crate, current): the "plumbing" — `CandidateConfig::build_app`
...
//!   runs generation 0 only (no variation/diagnosis/selection) and persists a
//!   report + per-case trajectory files.
//! - **E2** (subgraph diagnosis) → `subgraph.rs` + `failure_extractor.rs`.
...
pub use report::{CaseRecord, EvolutionReport};
