...
impl TaskStore {
    ...
    pub async fn complete_task(
        &self,
        id: &str,
        artifact: Option<crate::types::Artifact>,
    ) -> Result<Task> {
    ...
    pub async fn fail_task(&self, id: &str, error_message: &str) -> Result<Task> {
        self.add_message(id, Message::agent_text(error_message))
            .await?;
...
mod tests {
    ...
    async fn test_create_task() {
        let store = TaskStore::new();
        let task = store
            .create_task("task-001", Message::user_text("Hello"))
            .await
            .unwrap();
    ...
    async fn test_get_task() {
        let store = TaskStore::new();
        store
            .create_task("task-002", Message::user_text("Test"))
            .await
            .unwrap();
    ...
    async fn test_duplicate_task_id() {
        let store = TaskStore::new();
        store
            .create_task("task-003", Message::user_text("First"))
            .await
            .unwrap();
        let result = store
            .create_task("task-003", Message::user_text("Second"))
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_valid_state_transitions() {
        let store = TaskStore::new();
        store
            .create_task("task-004", Message::user_text("Work"))
            .await
            .unwrap();

        // Submitted → Working
        let task = store
            .transition_task("task-004", TaskState::Working)
            .await
            .unwrap();
        ...
        // Working → Completed
        let task = store
            .transition_task("task-004", TaskState::Completed)
            .await
            .unwrap();
    ...
    async fn test_invalid_state_transition() {
        let store = TaskStore::new();
        store
            .create_task("task-005", Message::user_text("Skip"))
            .await
            .unwrap();

        // Submitted → Completed is invalid
        let result = store
            .transition_task("task-005", TaskState::Completed)
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_cancel_task() {
        let store = TaskStore::new();
        store
            .create_task("task-006", Message::user_text("Cancel me"))
            .await
            .unwrap();
        store
            .transition_task("task-006", TaskState::Working)
            .await
            .unwrap();
    ...
    async fn test_complete_task_with_artifact() {
        let store = TaskStore::new();
        store
            .create_task("task-007", Message::user_text("Compute"))
            .await
            .unwrap();
        store
            .transition_task("task-007", TaskState::Working)
            .await
            .unwrap();

        let artifact = Artifact::text("result", "42");
        let task = store
            .complete_task("task-007", Some(artifact))
            .await
            .unwrap();
        assert_eq!(task.status.state, TaskState::Completed);
    ...
    async fn test_fail_task() {
        let store = TaskStore::new();
        store
            .create_task("task-008", Message::user_text("Fail"))
            .await
            .unwrap();
        store
            .transition_task("task-008", TaskState::Working)
            .await
            .unwrap();

        let task = store
            .fail_task("task-008", "Something went wrong")
            .await
            .unwrap();
    ...
    async fn test_require_input() {
        let store = TaskStore::new();
        store
            .create_task("task-009", Message::user_text("Input?"))
            .await
            .unwrap();
        store
            .transition_task("task-009", TaskState::Working)
            .await
            .unwrap();

        let task = store
            .require_input("task-009", Message::agent_text("Need more info"))
            .await
            .unwrap();
    ...
    async fn test_list_tasks() {
        let store = TaskStore::new();
        store
            .create_task("t1", Message::user_text("1"))
            .await
            .unwrap();
        store
            .create_task("t2", Message::user_text("2"))
            .await
            .unwrap();
    ...
    async fn test_remove_task() {
        let store = TaskStore::new();
        store
            .create_task("task-010", Message::user_text("Remove"))
            .await
            .unwrap();
