...
13. ✅ **真 tokenizer**（tiktoken-rs / huggingface-tokenizers），让 `truncate_tool_results` 真用 TokenCounter。——2026-08-24：workspace 接 `tiktoken-rs 0.12`（内置 o200k_base BPE，无运行时下载），`oneai-core` 新增 `real-tokenizer` feature + `TiktokenTokenCounter`（真 BPE 计数，per-provider 消息开销/上下文窗仍走原 profile/3 层 resolver）；`truncate_tool_results` 接 `Option<&dyn TokenCounter>`——有计数器时按 **token** 二分裁切（不再裸 `chars×4`）；`AppBuilder::default_token_counter()` 与自动装配路径默认换真计数器，session 两处 `ContextBudgetManager` 经 `with_token_counter` 接线；TUI/app-server/run/a2a 四入口 + `oneai token` CLI 全启用。
14. ✅ **StateGraph resume API** + 中途持久化 walk state（向 durable execution 走第一步）。——2026-08-24：`oneai-workflow` 新增 `checkpoint.rs`（`GraphCheckpoint` 快照 + `GraphCheckpointStore` trait + `InMemoryCheckpointStore`/`FileCheckpointStore`，run_id 清洗防路径穿越）；`StateGraphExecutor` 增 `execute_with_checkpoints`（每个迭代边界落盘）与 `resume`（校验 graph 名、清中断标志、从保存的 frontier/iterations/state 续走；完成即删 checkpoint）。interrupt 拒绝→重启→批准续走的端到端测试（含跨 store 实例"进程重启"场景）。
