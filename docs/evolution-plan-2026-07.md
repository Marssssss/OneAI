...
> - 取舍：(1) `.json.gz` 快照而非 `.tar.gz`——保 crate 免引 `tar` 依赖（供应链纪律），能力（可回滚 N 份快照）等同；(2) 归档被 workflow/cron 引用的技能只 warn 不重写（破坏性重写留后续 Stage）；(3) `referenced` 集起手空（cron 是 Phase 3.2，未到；`set_referenced` 是刷新钩）；(4) 跨 session cadence hydrate = Stage C。Stage C（LLM consolidation 默认关 + 跨 session cadence）待续。共 1456 workspace tests 绿，fmt+clippy 清。
