...
**EngineYield 全变体 1:1 映射 `AgentLoopObserver` 回调**：`TurnStart`/`IterationStart`/`StreamChunk`/`Thinking`/`DirectAnswer`/`ToolCalls`/`ToolResult`/`Delegate`/`DelegateComplete`/`ParadigmSwitch`/`ApprovalRequest`/`WorkingState`/`ContextAccounting`/`ContextAssembled`/`PlanUpdate`/`ToolsAdded`/`TokenUsage`/`Interrupted`/`Reflection`/`Error`/`TurnComplete` + 会话生命周期 `SessionCreated`/`SessionLoaded`/`SessionCleared`/`SessionDeleted`/`SessionEnded` + `/init`·`/compact` 结果 `InitResult`/`CompactResult`。`BusObserver` 把每个回调翻译成一个 yield 同步发出去（`broadcast::send` 是同步的，故能从 `AgentLoop` 调的同步 observer 方法里直发）。`Delegate` 带 `depends_on: Vec<String>`（DAG 委托边，`#[serde(default)]` 兼容旧生产者）；`ContextAssembled` 是轨迹时间轴渲染的上下文组装分段快照（issue #40）。
