...
- **数据**：`ProjectionStore` 把轨迹相关 `EngineYield` 折进 `trajectory` ledger——每迭代推理（`infer` 节点，由 `stream_chunk`/`thinking` 累积、边界 flush 进对应 `iteration_start` 节点，`inference` 事件把 API 请求/响应 + 推理耗时挂到该节点）、`context_assembled` 分段（按 key 缓存做 hash 去重；节点定位在对应 infer 节点**左侧**——上下文是推理的输入）、工具调用↔结果配对（按 `call_id` 回填结果+耗时）、审批请求（定位到其门控的工具节点左侧）、委托 DAG 边（`depends_on`）、working-state 快照、`interrupted`/`reflection`/`approval`/`error` 标记。
- **时间轴模型**：`trajectory/timeline.ts` 把扁平 ledger 折成泳道（lane 0=主 agent，每个委托 `task_id` 一条子泳道）+ fork/join/depends 边，按墙上时钟定位。`turn_start`/`turn_complete` **不是节点**——折成 `turns` 泳道标记（起始/结束虚线）。SVG 画布支持拖动平移、滚轮（以光标为锚点）缩放，**默认 fit-to-width**（整条过程一屏可见，节点加大到 ~10px 半径），用户拖走后「回到最新」重新跟随。
- **详情面板**：点击节点按类型渲染详情（`DetailPane`）——上下文分段（含总 token/段数/变更段统计，`context:` 前缀源标签）、推理/思考+token+耗时+**API 请求/响应**（采样参数、原始请求消息、模型响应、用量/缓存命中）、工具指令/结果+耗时、计划清单、委托摘要/关键发现、审批请求等。
